...
"""Subject-tracked batch crop/uncrop for video mask workflows."""
...
def _debug_summary(binary, boxes, scale, sel, info, output_size):
    ...
    for i in range(n):
        ...
        fr = []
        if b["x"] > 0:
            fr.append((xs.min() - b["x"]) / max(wx, 1e-6))
        if bx1 < img_w - 1:
            fr.append((bx1 - xs.max()) / max(wx, 1e-6))
        if b["y"] > 0:
            fr.append((ys.min() - b["y"]) / max(wy, 1e-6))
        if by1 < img_h - 1:
            fr.append((by1 - ys.max()) / max(wy, 1e-6))
...
def _plan_and_crop(original_images, masks, sel, p, divisible_by,
    ...
    if original_images.shape[0] != masks.shape[0]:
        raise ValueError(
            f"original_images ({original_images.shape[0]}) and masks "
            f"({masks.shape[0]}) must have the same frame count")
    if original_images.shape[1:3] != masks.shape[1:3]:
        raise ValueError(
            f"original_images ({original_images.shape[2]}x{original_images.shape[1]}) "
            f"and masks ({masks.shape[2]}x{masks.shape[1]}) must have the same dimensions")
    ...
    if full_frame:
        ...
        boxes, info = plan(binary, sel, p, divisible_by)
    ...
    report = _debug_summary(binary, boxes, p["crop_scale"],
...
_OUTPUTS = [
    ...
    io.String.Output(display_name="report", tooltip="Summary of the plan: chosen shape, box sizes, movement, and how much of the padding promise was kept."),
]


class MiniMaxH3_SubjectCrop(io.ComfyNode):
    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="MiniMaxH3_SubjectCrop",
            display_name="H3 Subject Crop",
            category=CATEGORY,
...
class MiniMaxH3_SubjectCropAdvanced(io.ComfyNode):
    @classmethod
    def define_schema(cls):
        def dial(name, **over):
            build = {
        ...
        zoomed_dials = [dial("crop_scale"), dial("min_padding_allowed"),
                        ...
                        dial("aspect_ratio"), dial("seamless_loop")]
        return io.Schema(
            node_id="MiniMaxH3_SubjectCropAdvanced",
            display_name="H3 Subject Crop (Advanced)",
            category=CATEGORY,
...
class MiniMaxH3_SubjectUncrop(io.ComfyNode):
    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="MiniMaxH3_SubjectUncrop",
            display_name="H3 Subject Uncrop",
            category=CATEGORY,
    ...
    def execute(cls, cropped_images, original_images, bboxes, feather, cropped_masks=None) -> io.NodeOutput:
        ...
        if n < max(counts.values()):
            ...
            logging.warning(
                f"MiniMaxH3 Subject Uncrop: frame counts differ ({got}), "
                f"using the first {n} frames of each")
