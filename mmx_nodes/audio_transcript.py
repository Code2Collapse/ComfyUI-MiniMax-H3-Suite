"""Transcribe a soundtrack, with word timings, for lipsync and shot markers.

PORTED FROM: comfyui-deno-custom-nodes (GPL-3.0). This pack is GPL-3.0, so the
licences agree; see CREDITS.md.

H3 generates audio and video jointly, and this pack's lipsync work drives
mouths from a control video. Neither of those knows WHEN a word happens.
Whisper's segment timings do - which is what turns a soundtrack into shot
markers, and into a timing reference for a mouth that has to hit a syllable.

Whisper itself is optional and imported lazily: it is not a ComfyUI
dependency, so its absence has to be a readable message at run time, never an
import failure that takes the node out of the menu.

CHANGED FROM UPSTREAM: the model cache lives on the MODULE, not on `self`.
Upstream relies on ComfyUI reusing the node instance between runs; this pack
registers classic nodes through an adapter that builds a fresh instance per
execution, so an instance cache would make "Keep loaded" silently reload a
...
def _import_whisper():
    try:
        ...
        if exc.name != "whisper":
            raise
        raise RuntimeError(
            "H3 Audio Transcript uses the optional openai-whisper package. "
...
def _import_torchaudio():
    try:
        ...
        raise RuntimeError(
            "H3 Audio Transcript needs torchaudio to resample source audio to 16 kHz. "
...
def _whisper_download_root() -> str:
    ...
    try:
        ...
        if exc.name != "folder_paths":
            raise
        raise RuntimeError(
            "H3 Audio Transcript could not locate ComfyUI's models directory. "
...
def _prepare_cuda_whisper_use() -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "H3 Audio Transcript could not prepare CUDA Smart Swap, so Whisper "
            "was not used. Free ComfyUI GPU models and try again."
        ) from exc
    _empty_cuda_cache_best_effort("cuda")


# The loaded Whisper model, shared across executions.
#
# On the MODULE, not on an instance: this pack builds a fresh node instance per
# execution (so per-call state cannot leak between runs), and an instance cache
# would therefore reload a multi-gigabyte model every single run while the
# "Keep loaded" option reported that it had kept it.
_MODEL_CACHE: dict[str, object] = {"model": None, "key": None}
...
class DenoAudioTranscript:
    ...
    def _get_or_load_model(model_name: str, device: str):
        ...
        if _MODEL_CACHE["model"] is not None and _MODEL_CACHE["key"] == key:
            if device == "cuda":
                try:
        ...
        old_device = _MODEL_CACHE["key"][1] if _MODEL_CACHE["key"] is not None else None
        had_cached_model = _MODEL_CACHE["model"] is not None or _MODEL_CACHE["key"] is not None
        if had_cached_model:
            _release_cached_model()
    ...
    def transcribe(
        ...
        try:
            ...
            if model_after_run == "Unload after run" or transcription_failed:
                ...
                if cleanup_device is None and _MODEL_CACHE["key"] is not None:
                    cleanup_device = _MODEL_CACHE["key"][1]
                had_cached_model = (
                    _MODEL_CACHE["model"] is not None
                    or _MODEL_CACHE["key"] is not None
...
# Registered through the V3 adapter, like the all-in-one Director: this pack
# uses comfy_entrypoint, and ComfyUI's loader takes the V1 branch and RETURNS
# if a pack exports NODE_CLASS_MAPPINGS - which would unregister every other
# node here. See mmx_nodes/director/v1_adapter.py.
try:
    import sys as _sys
    ...
    from mmx_nodes.director.v1_adapter import adapt as _adapt

    MiniMaxH3_AudioTranscript = _adapt(
        DenoAudioTranscript,
        node_id="MiniMaxH3_AudioTranscript",
        display_name="H3 Audio Transcript",
        category="MiniMax H3/Audio",
    )
except Exception as _exc:  # noqa: BLE001
    MiniMaxH3_AudioTranscript = None
    import logging as _lg
    _lg.getLogger(__name__).warning(
        "[MiniMaxSuite] Audio Transcript unavailable: %s", _exc)
