...
# PORTED FROM: MiniMaxH3-Director ("Muse Minimax Director") -- MIT, per the
# upstream README badge. See CREDITS.md.
#
# Changed here: a `prompt` input was ADDED (upstream takes every word from
# timeline_data, written by a canvas timeline this pack does not ship), the
# HTTP route registration is guarded so it cannot explode at import time, PyAV
# is imported defensively, and the node id is namespaced to this pack.
...
# PyAV decodes the reference video/audio clips. It is not a ComfyUI
# dependency, so its absence must not take this node out of the menu - the
# generator itself does not need it.
try:
    import av
    _AV_ERROR = None
...
# Registered through a guard rather than a bare decorator: a decorator runs
# at IMPORT time, and PromptServer.instance only exists once the server has
# been built. Unguarded, importing this module before the server is up raises
# AttributeError and the node silently vanishes from /object_info.
def _route(method: str, path: str):
    def decorate(fn):
        server = getattr(PromptServer, "instance", None)
        routes = getattr(server, "routes", None) if server is not None else None
        register = getattr(routes, method, None) if routes is not None else None
        return fn if register is None else register(path)(fn)
    return decorate


@_route("post", "/muse_minimax_director/analyze_character")
...
def _seed_prompt_segment(tdata: dict, prompt: str) -> dict:
    """Make a bare prompt behave as a one-shot timeline.

    ADDED (not upstream). The chunker works on `segments`; with none it has
    nothing to say and the generator runs on an empty prompt. A timeline that
    already has shots is left alone - the timeline is the richer description
    and must win.
    """
    text = (prompt or "").strip()
    if not text or tdata.get("segments"):
        return tdata
    tdata["segments"] = [{"description": text, "weight": 1}]
    return tdata
...
class MuseMinimaxDirector:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                # ADDED (not upstream). Upstream has no prompt socket: every word
                # comes from timeline_data, which its canvas timeline writes. This
                # pack does not ship that front-end, so without this the node
                # would generate from an empty prompt. When the timeline has no
                # segments, this becomes the single segment.
                "prompt": ("STRING", {"default": "", "multiline": True, "tooltip":
                    "What the video is. Used when the timeline has no shots of its "
                    "own - then the whole duration is one shot described by this. "
                    "With a timeline loaded the timeline's own shots win and this "
                    "is ignored."}),
    ...
    def execute(self, mode, model, clip, vae, audio_vae, aspect_ratio, megapixels, multiple, resize_method,
                ...
                prompt="", model_fl2va=None):
        tdata = _parse_timeline(timeline_data)
        tdata = _seed_prompt_segment(tdata, prompt)
...
NODE_CLASS_MAPPINGS = {
    "MiniMaxH3_DirectorAllInOne": MuseMinimaxDirector,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "MiniMaxH3_DirectorAllInOne": "H3 Director (All-in-One, chunked)",
}
