...
# --------------------------------------------------------------------------------------
# Route registration, deferred rather than assumed
# --------------------------------------------------------------------------------------
#
# CHANGED FROM UPSTREAM. Upstream decorates these with the server's route
# registrar directly. That registrar hangs off a class attribute which is
# only set once the server has been constructed, and a decorator runs at IMPORT
# time - so importing this module before the server exists raises
# AttributeError and takes every Director node down with it, invisibly. Inside
# a live ComfyUI that never happens, which is why upstream does not see it;
# here it made the whole subpackage uncollectable and untestable.
#
# This registers the route when it can and returns the function unchanged when
# it cannot, so the node classes import either way.

_ROUTES_DEFERRED: list[str] = []


def _route(method: str, path: str):
    """Register an HTTP route if the server is up; otherwise skip it quietly."""
    def decorate(fn):
        ...
        if register is None:
            _ROUTES_DEFERRED.append(path)
            return fn
        return register(path)(fn)
    return decorate


@_route("get", "/minimax_director_check_file")
...
@_route("post", "/minimax_director/compile_prompt")
...
@_route("post", "/minimax_director/probe_video")
...
@_route("get", "/minimax_director_get_audio")
...
@_route("get", "/minimax_director_open_folder")
...
@_route("post", "/minimax_director_upload_chunk")
...
@_route("post", "/minimax_director/analyze_character")
...
@_route("post", "/minimax_director/unload_ollama")
