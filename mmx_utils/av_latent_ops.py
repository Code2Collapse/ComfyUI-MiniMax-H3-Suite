"""H3 AV latent operations - pack / split / merge / continuation.

PORTED FROM: third_party/ComfyUI-MiniMax-H3-LongMedia/latent_ops.py
             (ComfyUI-MiniMax-H3-LongMedia). Ported verbatim apart from this
             header; the owner authorised direct porting of third_party code.

WHY IT PORTS CLEANLY: this module imports ONLY torch. H3's NestedTensor is
passed in as a `nested_factory` argument rather than imported, so none of the
arithmetic here depends on the ComfyUI build shipping comfy.ldm.minimax. That
matters: the installed ComfyUI on this machine does not have it, and a
module-scope H3 import is what silently unregistered six nodes during an
earlier port. The lazy import lives in the NODE layer (mmx_nodes/continuation.py),
not here.

Key arithmetic, consistent with mmx_utils/h3_constants.py:
  FPS 24, AUDIO_LATENT_FPS 40 - the same 40 Hz clock the temporal RoPE runs on
  align_frame_count  snaps to the 17k+5 grid
  video_latent_t / audio_latent_t / frame_count_from_video_t  convert between
  pixel frames and latent rows in both directions
"""
