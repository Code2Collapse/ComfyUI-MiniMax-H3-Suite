# PORTED FROM: MaskVidExperiments (third_party/MaskVidExperiments/planner.py) by drozbay
...
def _solve_tracked(pre, img_w, img_h, p):
    ...
    for i in range(m):
        ...
        lp.row([x, W], [-1, -1], -(bx1[i] + 1))
        lp.row([x, W], [1, 1], img_w)
        lp.row([y, H], [-1, -1], -(by1[i] + 1))
        lp.row([y, H], [1, 1], img_h)
        lp.row([x, vx], [1, -1], pre["lx"][i])
        lp.row([x, W, vx], [-1, -1, -1], -pre["rx"][i])
...
def _solve_zoomed_at(pre, ar, img_w, img_h, p):
    ...
    for i in range(m):
        ...
        r = lp.off["r"] + i
        ux, uy = lp.off["ux"] + i, lp.off["uy"] + i
        lp.row([x, h], [-1, -ar], -(bx1[i] + 1))
        lp.row([x, h], [1, ar], img_w)
        lp.row([y, h], [-1, -1], -(by1[i] + 1))
        lp.row([y, h], [1, 1], img_h)
        lp.row([x, vx], [1, -1], pre["lx"][i])
