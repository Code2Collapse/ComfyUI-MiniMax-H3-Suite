"""MiniMax H3 Director - the planner, checked offline.

PORTED FROM: ComfyUI-MiniMaxH3-Director (GPL-3.0). Upstream's own suite, run
under pytest instead of as a script.

`minimax_plan` imports nothing outside the standard library on purpose, so
this needs no ComfyUI, no server and no weights. Every consumer of the planner
depends on it agreeing with itself: the Director encodes what the live
compiled-prompt panel shows, and the panel is only trustworthy because both
come through here.
"""
...
# Loaded by path, as upstream does, rather than imported as
# mmx_nodes.director.minimax_plan - the planner is deliberately free of
# ComfyUI imports and loading it directly keeps this suite runnable with
# nothing but the standard library, which is the property that makes it
# useful.
HERE = os.path.dirname(os.path.abspath(__file__))
PLAN_PY = os.path.join(os.path.dirname(HERE), "mmx_nodes", "director",
                       "minimax_plan.py")
spec = importlib.util.spec_from_file_location("minimax_plan", PLAN_PY)
...
def test_the_suite_actually_ran():
    ...
    assert len(_results) >= 140, (
        f"only {{len(_results)}} checks ran; the suite was expected to have at "
        f"least 140. Something stopped the module part-way through.")
