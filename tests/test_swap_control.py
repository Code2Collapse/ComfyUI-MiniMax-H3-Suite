...
The load-bearing property, again: no jaw edge exists anywhere in the table, so
the dupe's face contour cannot reach the model however the node is configured.
...
def kps(frames=1, w=256, h=256):
    ...
    a[..., 0] = rng.uniform(20, w - 20, size=(frames, N_WHOLEBODY))
    a[..., 1] = rng.uniform(20, h - 20, size=(frames, N_WHOLEBODY))
    a[..., 2] = 1.0
    return a
...
def test_there_is_no_jaw_edge_table_at_all():
    """Not 'filtered out' - absent. A filter is something to forget."""
    assert "jaw" not in GROUP_EDGES


@pytest.mark.parametrize("scope", sorted(SWAP_SCOPES))
def test_no_scope_draws_an_edge_touching_the_jaw(scope):
    jaw = set(range(*GROUPS["jaw"]))
    for (a, b), _ in scope_edges(scope):
        assert a not in jaw and b not in jaw, (
            f"scope {scope!r} draws edge {(a, b)}, which touches the jaw")
...
def test_every_edge_index_is_in_range():
    for group, edges in GROUP_EDGES.items():
        for a, b in edges:
            assert 0 <= a < N_WHOLEBODY and 0 <= b < N_WHOLEBODY, (group, a, b)
...
def test_lips_scope_draws_only_in_the_mouth_region():
    """A tight, checkable claim: put the mouth in a known box, everything else
    far away, and assert nothing is drawn outside the box."""
